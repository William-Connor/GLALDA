...
if __name__ == "__main__":
    def run_combined_parameter_sweep(k_values, t_values, alph_values, file_name):
        best_auc = 0
        best_results = None
        best_params = None

        with open(file_name, 'w') as file:
            file.write('k t alph AUC AUPR ACC P R F1\n')
            print("表头保存成功！")
        ...
        for k in k_values:
            for t in t_values:
                for alph in alph_values:
                    try:
                        main(k, t, alph)  # 假设main函数可以同时接收k, t, alph
                        ...
                        if current_auc > best_auc:
                            ...
                            best_params = (k, t, alph)

                        with open(file_name, 'a') as file:
                            file.write(f'{k} {t} {alph} ' + " ".join(last_line) + '\n')
                            print(f"参数组合 k={k}, t={t}, alph={alph} 及其指标保存成功！")

                    except Exception as e:
                        print(f"处理参数组合 k={k}, t={t}, alph={alph} 时发生错误: {str(e)}")

        # print(f'Best AUC: {best_auc}')
        # print(f'Best Parameters: k={best_params[0]}, t={best_params[1]}, alph={best_params[2]}')
    ...
    run_combined_parameter_sweep(k_range, t_range, alph_range, 'ktalph_results.txt')
