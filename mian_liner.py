...
def main(config):
    ...
    # run=wandb.init()
    # 复杂图的临界矩阵
    print(f"Testing with hid1 = {config['hid1']}, hid2 = {config['hid2']}")
...
####miRNAz特征视图
    ...
    with open('best_output.txt', 'a+') as f:
        f.write(
            f'{best_result[0]} {best_result[1]} {best_result[2]} {best_result[3]} {best_result[4]} {best_result[5]}\n')
    #methods.save_to_excel(best_labels, best_preds,max_AUC,seed=9854)

if __name__ == "__main__":

    # 定义hid1和hid2的值范围
    hid1_values = [16, 32, 64,128,256,512]
    hid2_values = [16, 32, 64,128,256,512]



    # 遍历所有可能的hid1和hid2值
    for hid1 in hid1_values:
        for hid2 in hid2_values:
            config['hid1'] = hid1
            config['hid2'] = hid2

            # 调用main函数，传递更新后的config
            main(config)
            # 读取并记录AUC
            ...
            with open('liner.txt', 'a') as f:
                f.write(f'hid1={hid1} hid2={hid2} ')
