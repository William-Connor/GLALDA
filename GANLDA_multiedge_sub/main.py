...
if __name__ == "__main__":
    att_drop_rates = [0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5] # 以你需要的att_drop_rate值列表为例
    best_auc = 0
    best_results = None
    with open('dropout.txt', 'w') as k_file:
        k_file.write('dropout AUC AUPR ACC P R F1\n')  # 写入表头
        print("表头保存成功！")
    for att_drop_rate in att_drop_rates:
        config['att_drop_rate'] = att_drop_rate  # 动态设置config中的att_drop_rate

        # 调用main()函数
        main(config)  # 你可以在main()函数中返回最佳结果
        ...
        # 保存每个att_drop_rate对应的最佳结果

        with open('dropout.txt', 'a') as k_file:
            k_file.write(f'{att_drop_rate} ')  # 先保存k值
            print("dropout 保存成功！")
            k_file.write(" ".join(last_line) + '\n')  # 再保存指标
            print("dropout对应指标保存成功！")
