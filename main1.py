...
def main():
    ...
    A = np.load('data/ours/' + wandb.config.dataset + '/A_' + str(wandb.config.fold) + '.npy')
...
####dis特征视图
    ...
    positive_ij = np.load('data/ours/' + wandb.config.dataset + '/positive_ij.npy')
    # 96183 * 2里面存储的id，（i，j），ndarray
    negative_ij = np.load('data/ours/' + wandb.config.dataset + '/negative_ij.npy')
    positive5foldsidx = np.load('data/ours/' + wandb.config.dataset + '/positive5foldsidx.npy', allow_pickle=True)
    negative5foldsidx = np.load('data/ours/' +wandb.config.dataset + '/negative5foldsidx.npy', allow_pickle=True)

    ##测试集id
    positive_test_ij = positive_ij[positive5foldsidx[wandb.config.fold]['test']]
    # 94026  2
    negative_test_ij_tmp = negative_ij[negative5foldsidx[wandb.config.fold]['test']]
    # 生成随机索引
    ...
    # 训练集id
    # 2157*2
    positive_train_ij = positive_ij[positive5foldsidx[wandb.config.fold]['train']]
    negative_train_ij = negative_ij[negative5foldsidx[wandb.config.fold]['train']]
    # 4314个标签
    train_target = torch.Tensor([1] * len(positive_train_ij) + [0] * len(negative_train_ij))

    # ganlda model
    ganlda_model = GANLDAModel(wandb.config.gan_in_channels,wandb.config.hid, wandb.config.gan_out_channels, wandb.config.n_head, wandb.config.att_drop_rate,wandb.config, gpu)
    # wandb.watch(ganlda_model, log='all', log_graph=True)

    optimizer = torch.optim.Adam(ganlda_model.parameters(), lr=wandb.config.lr,
                                 weight_decay=wandb.config.weight_decay)  # 更新参数
    ...
    for epoch in range(1, 1000):
        ...
        out, loss = methods.train(positive_train_ij, negative_train_ij, train_target, ganlda_model, optimizer, H,
                                  A,lnc_dis_fea,lnc_dis_view,lnc_feature,lnc_feature_view,dis_feature,dis_feature_view,wandb.config,gpu)
        ...
        ganlda_model.eval()
        pred = ganlda_model(positive_test_ij, negative_test_ij, H, A,lnc_dis_fea,lnc_dis_view,lnc_feature,lnc_feature_view,dis_feature,dis_feature_view,wandb.config,gpu)
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(
        ...
        project='ganlda_model_cross_edge'
    )
