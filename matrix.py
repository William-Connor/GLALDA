...
def max_similarity(similarity_matrices):
    # 将所有矩阵堆叠为一个三维数组
    stacked_matrices = np.stack(similarity_matrices)

    # 沿着第一个轴（索引0），找到沿着第三个轴（索引2）的最大值
...
def snf_fusion(lnc_sim, cos_sim, kernel_sim, k, t, alpha):
    ...
    # affinity_matrices = [lnc_sim, cos_sim]
...
def extract_and_fuse_matrices(adj,k, t, alpha):
    ...
    lnc_cosine_sim = calculate_cosine_similarity(lnc_dis)
    dis_cosine_sim = calculate_cosine_similarity(lnc_dis.T)
    mi_cosine_sim = calculate_cosine_similarity(dis_mi.T)
    ...
    def fuse_similarity_matrices(sim1, sim2, sim3):
        affinity_matrices = [sim1, sim2, sim3]
        return snf.snf(sim1, sim2, sim3,K=k, t=t, alpha=alpha)
    def max_similarity_matrices(sim1, sim2, sim3):
        affinity_matrices = [sim1, sim2, sim3]
        return max_similarity(affinity_matrices)
    lnc_fused_sim=fuse0_similarity_matrices(lnc_sim,lnc_cosine_sim,lnc_kernel_sim)
    dis_fused_sim=fuse0_similarity_matrices(dis_sim,dis_cosine_sim,dis_kernel_sim)
    mi_fused_sim=fuse0_similarity_matrices(mi_sim,mi_cosine_sim,mi_kernel_sim)
