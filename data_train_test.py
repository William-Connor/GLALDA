...
def get_data(Ai, ij):
    data = []
    for item in ij:
        feature = np.array([Ai[0][item[0]], Ai[0][item[1]]])#2*1147
...
class myDataset(Dataset):
    def __init__(self, dataset, fold, dimension, mode='train') -> None:
        ...
        if mode == 'test':
            ...
            #将形状从(batch_size, seq_len, feature_dim)转换为(batch_size, feature_dim, seq_len)。
            #94566 1147 6
            self.data = torch.cat((positive_test_data, negative_test_data)).transpose(1, 2)
            #94566个标签
            self.target = torch.Tensor([1] * len(positive_test_ij) + [0] * len(negative_test_ij))#标签
...
def prep_dataloader(dataset, fold, mode, dimension,  n_jobs=0):
